...
def init_logger(fname=None, lev=None):
    ...
    level = settings.get_setting("debug")

    # Logger propio de plataforma (kodi) se formatea el mensaje y se pasa al modulo de platformlogger
    if platform_logger and not (lev == 4 or fname):
        from platformcode import platformlogger
        logger_object = platformlogger.logger_object

    # Si está configurado el logger propio o estamos en nivel 4 (modo reporte) se usa el logger propio
    else:
        filename = fname or 'mediaexplorer.log'
        ...
        handler.setFormatter(
            logging.Formatter(
                '%(asctime)8s %(levelname)-5s %(filename)-30s %(funcName)-20s %(lineno)-4s | %(message)s',
                '%H:%M:%S'
            )
        )
...
def format_message(data):
    ...
    Formatea el mensaje para mostrarlo en el log:
        ...
        - Se ocultan las contraseñas siempre que el nombre de la varible que la contiene contenga la palabra password
