...
def create(text, code="", title="", language="text", privacy="private", json=False):
    ...
    try:
        ...
        return r if json else r['id']

    except:
        return {'ok': False, 'reason':reason} if json else None
...
def read(id, json=False):
    ...
    try:
        ...
        return r if json else r['snippet']
    except:
        return {'ok': False, 'reason': 'Unknown'} if json else None
...
def lock(id, code):
    try:
        ...
        if scrapertools.find_single_match(r,'class="errors"'):
            raise()
