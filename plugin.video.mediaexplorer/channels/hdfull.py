...
class Decoder:
    def __init__(self):
        ...
        self.varname, values = re.compile('var (_0x[0-f]+)=(\[.*?\])').findall(self.code)[0]
        number = eval(re.compile('\}\(%s,(0x[0-f]+)\)\)' % self.varname).findall(self.code)[0])
    ...
    def decode(self):
        self.values = [base64.b64decode(a) for a in self.values]

    def compose(self):
        result = {}
        for prov in re.compile("p\[0x[0-f]+\]={}.*?\['l'\].*?};").findall(self.code):
            code = re.compile('p\[(0x[0-f]+)\]={};').findall(prov)[0]
            t = re.compile("p\[%s\]\['t'\]='([^']+)';" % code).findall(prov)[0]
            link = self.values[eval(re.compile("return _0x[0-f]+\('([^']+)'\)").findall(prov)[0])] + '%s'
