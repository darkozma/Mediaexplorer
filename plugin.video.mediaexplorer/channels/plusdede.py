...
QLT = Qualities({
    ...
    Qualities.scr: ['Ts Screener', 'Tc Screener'],
})
...
def mainlist(item):
    logger.trace()
    itemlist = []
    item.fanart = "fanart/plusdede.png"
...
def menupeliculas(item):
    ...
    itemlist.append(item.clone(
        label='Novedades',
        action='movies',
        url='https://www.plusdede.com/pelis',
        content_type='movies',
        type="item",
        group=True
    ))
    ...
    itemlist.append(item.clone(
        label='Mas vistas',
        action='movies',
        url='https://www.plusdede.com/pelis/top',
        content_type='movies',
        type="item",
        group=True
    ))
    ...
    itemlist.append(item.clone(
        label='Todas',
        action='movies',
        url='https://www.plusdede.com/pelis/all',
        content_type='movies',
        type="item",
        group=True
    ))
...
def menuseries(item):
    ...
    itemlist.append(item.clone(
        label='Novedades',
        action='tvshows',
        url='https://www.plusdede.com/series',
        type="item",
        group=True,
    ...
    itemlist.append(item.clone(
        label='Mas vistas',
        action='tvshows',
        url='https://www.plusdede.com/series/top',
        type="item",
        group=True,
        content_type='tvshows'
    ))
    ...
    itemlist.append(item.clone(
        label='Todas',
        action='tvshows',
        url='https://www.plusdede.com/series/all',
        type="item",
        group=True,
        content_type='tvshows'
    ))
...
def movies(item):
    ...
    for result in re.compile(patron, re.DOTALL).finditer(data):
        itemlist.append(item.clone(
            ...
            url='https://www.plusdede.com/aportes/4/%s' % result.group('id'),  # result.group('url'),
...
def login():
    ...
    data = httptools.downloadpage('https://www.plusdede.com/login').data
    ...
    if username and username == settings.get_setting('user', __file__):
        ...
        httptools.downloadpage('https://www.plusdede.com/logout')
        data = httptools.downloadpage('https://www.plusdede.com/login').data
        captcha = scrapertools.find_single_match(data, '<img src="([^"]+)" alt="captcha">')
        _token = scrapertools.find_single_match(data, '<input name="_token" type="hidden" value="([^"]+)">')
    ...
    data = httptools.downloadpage('https://www.plusdede.com/login', post=post).data

    post = {'setting_4': 1,
            '_token': _token
            }

    httptools.downloadpage('https://www.plusdede.com/config/setsettings', post=post)

    if not '"redirect":"https:\\/\\/www.plusdede.com"' in data:
        platformtools.dialog_notification('Pordede', 'Login incorrecto')
        return False
    else:
        return True
...
def movie_search(item):
    ...
    data = httptools.downloadpage(item.url or 'https://www.plusdede.com/search/%s' % item.query).data
...
def findvideos(item):
    ...
    data = httptools.downloadpage(urlparse.urljoin('https://www.plusdede.com/', item.url)).data
